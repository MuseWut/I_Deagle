...
void svp_simple_021_001_init() {
  svp_simple_021_001_tc_block_rcvd_bytes_ch1 = rand();  //w1
...
void svp_simple_021_001_RecvTcBlock() {
  ...
  if (svp_simple_021_001_tc_block_rcvd_bytes_ch1 >= 16) { // r1
    svp_simple_021_001_tc_block_rcvd_bytes_ch1 = 0;    //   w2
    int i;
    for (i = 0; i < 256; i++) {
  ...
  for (ichan = 0; ichan < 2; ichan++) {
    if (ichan == 0) {
      ...
      svp_simple_021_001_tc_block_rcvd_bytes =
          svp_simple_021_001_tc_block_rcvd_bytes_ch1; //r2
    } else {
...
// 轩宇
// 4.svp_simple_021_001_tc_block_rcvd_bytes_ch1 <R,#65>, <W,#79>, <R,#65> 然而并不会，因为65行所在分支语句只会执行一次
